...
import "./hermesswap/interfaces/IHermesRouter01.sol";
...
contract HermesRoll is Ownable {
    ...
    function pairForOldRouter(address tokenA, address tokenB) internal view returns (address pair) {
        (address token0, address token1) = HermesLibrary.sortTokens(tokenA, tokenB);
        pair = address(
            uint256(
                keccak256(
                    abi.encodePacked(
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = HermesLibrary.getReserves(address(factory), tokenA, tokenB);
