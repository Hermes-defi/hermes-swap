...
describe("Stable Hermes Staking", function () {
  before(async function () {
    this.StableHermesStakingCF = await ethers.getContractFactory(
      "StableHermesStaking"
    );
    this.HermesTokenCF = await ethers.getContractFactory("Hermes");
    ...
    this.hermesMaker = this.signers[4];
    this.penaltyCollector = this.signers[5];
  });

  beforeEach(async function () {
    this.rewardToken = await this.HermesTokenCF.deploy();
    ...
    await this.rewardToken.mint(
      this.hermesMaker.address,
      ethers.utils.parseEther("1000000")
    ); // 1_000_000 tokens

    this.stableHermesStaking = await hre.upgrades.deployProxy(
      this.StableHermesStakingCF,
      [
    ...
    await this.hermes
      .connect(this.alice)
      .approve(
        this.stableHermesStaking.address,
  ...
  describe("should allow deposits and withdraws", function () {
    it("should allow deposits and withdraws of multiple users", async function () {
      ...
      expect(await this.hermes.balanceOf(this.carol.address)).to.be.equal(
      ...
      await this.stableHermesStaking.connect(this.bob).withdraw("1");
      expect(await this.hermes.balanceOf(this.bob.address)).to.be.equal(
    ...
    it("should update variables accordingly", async function () {
      await this.stableHermesStaking.connect(this.alice).deposit("1");

      await this.rewardToken
        .connect(this.hermesMaker)
        .transfer(this.stableHermesStaking.address, ethers.utils.parseEther("1"));
      expect(
        await this.rewardToken.balanceOf(this.stableHermesStaking.address)
      ).to.be.equal(ethers.utils.parseEther("1"));
      expect(
        await this.stableHermesStaking.lastRewardBalance(this.rewardToken.address)
      ).to.be.equal("0");

      await increase(86400);
      expect(
        await this.stableHermesStaking.pendingReward(
          this.alice.address,
          this.rewardToken.address
        )
      ).to.be.equal(ethers.utils.parseEther("1"));
      ...
      await this.stableHermesStaking.updateReward(this.rewardToken.address);
      expect(
        await this.stableHermesStaking.pendingReward(
          this.alice.address,
          this.rewardToken.address
    ...
    it("should allow deposits and withdraws of multiple users and distribute rewards accordingly", async function () {
      ...
      await this.rewardToken
        .connect(this.hermesMaker)
        .transfer(this.stableHermesStaking.address, ethers.utils.parseEther("6"));
      await this.stableHermesStaking.updateReward(this.rewardToken.address);
      await increase(86400);
      ...
      await this.stableHermesStaking.connect(this.bob).withdraw("0");
      // reward = accRewardBalance * carolShare / PRECISION
    ...
    it("should distribute token accordingly even if update isn't called every day", async function () {
      ...
      await this.rewardToken
        ...
        .transfer(this.stableHermesStaking.address, ethers.utils.parseEther("1"));
      await increase(86400);
      await this.stableHermesStaking.connect(this.alice).withdraw(0);
    ...
    it("should allow deposits and withdraws of multiple users and distribute rewards accordingly even if someone enters or leaves", async function () {
      ...
      await this.rewardToken
        .connect(this.hermesMaker)
        .transfer(this.stableHermesStaking.address, ethers.utils.parseEther("4"));
      await increase(86400);
    ...
    it("pending tokens function should return the same number of token that user actually receive", async function () {
      ...
      ); // We send 100 Tokens to sHermes's address

      const pendingReward = await this.stableHermesStaking.pendingReward(
        this.alice.address,
        this.rewardToken.address
      );
      await this.stableHermesStaking.connect(this.alice).withdraw("0"); // Alice shouldn't receive any token of the last reward
      expect(await this.hermes.balanceOf(this.alice.address)).to.be.equal(
        ethers.utils.parseEther("700")
      );
      expect(await this.rewardToken.balanceOf(this.alice.address)).to.be.equal(
    ...
    it("should allow rewards in HERMES and USDC", async function () {
      await this.stableHermesStaking
        .connect(this.alice)
      ...
      // accHermesBalance = 0
      // reward = 0
      expect(await this.hermes.balanceOf(this.alice.address)).to.be.equal(0);

      await this.stableHermesStaking.addRewardToken(this.hermes.address);
      await this.hermes.mint(
        this.stableHermesStaking.address,
      ...
      // accHermesBalance = hermesBalance * PRECISION / totalStaked
      //                  = 6e18 * 1e24 / 291e18
      //                  = 0.002061855670103092783505e24
      // reward = accHermesBalance * aliceShare / PRECISION
      //        = accHermesBalance * 970e18 / 1e24
      //        = 1.999999999999999999e18
      expect(await this.hermes.balanceOf(this.bob.address)).to.be.closeTo(
        ethers.utils.parseEther("2"),
        ethers.utils.parseEther("0.0001")
      );
      ...
      //        = accHermesBalance * 970e18 / 1e24
      //        = 1.999999999999999999e18
      expect(await this.hermes.balanceOf(this.alice.address)).to.be.closeTo(
        ethers.utils.parseEther("2"),
        ethers.utils.parseEther("0.0001")
      );
    });
    ...
    it("rewardDebt should be updated as expected, alice deposits before last reward is sent", async function () {
      let token1 = await this.HermesTokenCF.deploy();
      await this.stableHermesStaking.addRewardToken(token1.address);

      await this.stableHermesStaking.connect(this.alice).deposit(1);
      await this.stableHermesStaking.connect(this.bob).deposit(1);

      await token1.mint(
        this.stableHermesStaking.address,
        ethers.utils.parseEther("1")
      );
      await this.stableHermesStaking.connect(this.alice).withdraw(1);
      ...
      await this.stableHermesStaking.removeRewardToken(token1.address);
    });
    ...
    it("should allow adding and removing a rewardToken, only by owner", async function () {
      let token1 = await this.HermesTokenCF.deploy();
      await expect(
        this.stableHermesStaking.connect(this.alice).addRewardToken(token1.address)
      ).to.be.revertedWith("Ownable: caller is not the owner");
      expect(
        await this.stableHermesStaking.isRewardToken(token1.address)
      ).to.be.equal(false);
      expect(await this.stableHermesStaking.rewardTokensLength()).to.be.equal(1);

      await this.stableHermesStaking
        .connect(this.dev)
        .addRewardToken(token1.address);
      await expect(
        this.stableHermesStaking.connect(this.dev).addRewardToken(token1.address)
      ).to.be.revertedWith("StableHermesStaking: token can't be added");
      expect(
        await this.stableHermesStaking.isRewardToken(token1.address)
      ).to.be.equal(true);
      expect(await this.stableHermesStaking.rewardTokensLength()).to.be.equal(2);
    ...
    it("should allow setting a new deposit fee, only by owner", async function () {
      ...
      expect(
        await this.hermes.balanceOf(this.penaltyCollector.address)
      ).to.be.equal(ethers.utils.parseEther("3"));

      await expect(
        this.stableHermesStaking.connect(this.alice).setDepositFeePercent("0")
      ).to.be.revertedWith("Ownable: caller is not the owner");
      await expect(
        this.stableHermesStaking
          .connect(this.dev)
          .setDepositFeePercent(ethers.utils.parseEther("0.5").add("1"))
      ).to.be.revertedWith(
        "StableHermesStaking: deposit fee can't be greater than 50%"
      );

      await this.stableHermesStaking
        .connect(this.dev)
        .setDepositFeePercent(ethers.utils.parseEther("0.49"));
      expect(await this.stableHermesStaking.depositFeePercent()).to.be.equal(
    ...
    it("should allow emergency withdraw", async function () {
      ...
      await this.stableHermesStaking.connect(this.alice).emergencyWithdraw(); // Alice shouldn't receive any token of the last reward
      expect(await this.hermes.balanceOf(this.alice.address)).to.be.equal(
      ...
      const userInfo = await this.stableHermesStaking.getUserInfo(
        this.stableHermesStaking.address,
        this.rewardToken.address
      );
