...
/// @title Money Maker
/// @author Hermes
/// @notice MoneyMaker receives 0.05% of the swaps done on Hermes in the form of an LP. It swaps those LPs
...
contract MoneyMaker is Ownable {
    ...
    /// @param _wone The address of wone
    constructor(
        address _factory,
        address _bar,
        address _tokenTo,
    ...
    function setBridge(address token, address bridge) external onlyAuth {
        // Checks
        require(
            token != tokenTo && token != wone && token != bridge,
    ...
    function _convertStep(
        ...
        if (token0 == token1) {
            ...
            if (token0 == tokenTo) {
                ...
                tokenOut = _toToken(wone, amount, slippage);
            } else {
            ...
            tokenOut = _toToken(
                wone,
                _swap(token1, wone, amount1, address(this), slippage).add(
                    amount0
                ),
                slippage
            );
            ...
            tokenOut = _toToken(
                wone,
                _swap(token0, wone, amount0, address(this), slippage).add(
                    amount1
                ),
                slippage
            );
    ...
    function getAmountOut(
        ...
        require(
            ...
            "MoneyMaker: INSUFFICIENT_LIQUIDITY"
        );
