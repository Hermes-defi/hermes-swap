...
/// @notice The (older) MasterChefHermesV2 contract gives out a constant number of HERMES tokens per block.
/// It is the only address with minting rights for HERMES.
/// The idea for this MasterChefHermesV3 (MCJV3) contract is therefore to be the owner of a dummy token
/// that is deposited into the MasterChefHermesV2 (MCJV2) contract.
/// The allocation point for this pool on MCJV3 is the total allocation point for all pools that receive double incentives.
contract MasterChefHermesV3 is Ownable, ReentrancyGuard {
    ...
    /// Also known as the amount of HERMES to distribute per block.
    struct PoolInfo {
        IERC20 lpToken;
    ...
    /// @notice Address of HERMES contract.
    IERC20 public immutable HERMES;
    ...
    /// @param _MASTER_CHEF_V2 The HermesSwap MCJV2 contract address.
    /// @param _hermes The HERMES token contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF_V2` MCJV2. This is required because MCJV2 holds the minting rights for HERMES.
    ...
    function add(
        ...
        if (address(_rewarder) != address(0)) {
            _rewarder.onHermesReward(address(0), 0);
        }
    ...
    /// @notice Update the given pool's HERMES allocation point and `IRewarder` contract. Can only be called by the owner.
    ...
    function set(
        ...
        if (overwrite) {
            _rewarder.onHermesReward(address(0), 0); // sanity check
    ...
    /// @notice View function to see pending HERMES on frontend.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _user Address of user.
    /// @return pendingHermes HERMES reward for a given user.
        ...
        )
    {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            uint256 hermesReward = secondsElapsed.mul(hermesPerSec()).mul(pool.allocPoint).div(totalAllocPoint);
            accHermesPerShare = accHermesPerShare.add(hermesReward.mul(ACC_TOKEN_PRECISION).div(lpSupply));
        }
        pendingHermes = user.amount.mul(accHermesPerShare).div(ACC_TOKEN_PRECISION).sub(user.rewardDebt);
    ...
    /// @notice Calculates and returns the `amount` of HERMES per block.
    function hermesPerSec() public view returns (uint256 amount) {
        uint256 total = 1000;
        ...
        uint256 lpShare = MASTER_CHEF_V2.hermesPerSec().mul(lpPercent).div(total);
    ...
    function updatePool(uint256 pid) public {
        ...
        if (block.timestamp > pool.lastRewardTimestamp) {
            ...
            if (lpSupply > 0) {
                uint256 secondsElapsed = block.timestamp.sub(pool.lastRewardTimestamp);
                uint256 hermesReward = secondsElapsed.mul(hermesPerSec()).mul(pool.allocPoint).div(totalAllocPoint);
                pool.accHermesPerShare = pool.accHermesPerShare.add((hermesReward.mul(ACC_TOKEN_PRECISION).div(lpSupply)));
            }
            pool.lastRewardTimestamp = block.timestamp;
            poolInfo[pid] = pool;
            emit UpdatePool(pid, pool.lastRewardTimestamp, lpSupply, pool.accHermesPerShare);
        }
    }

    /// @notice Deposit LP tokens to MCJV3 for HERMES allocation.
    /// @param pid The index of the pool. See `poolInfo`.
    ...
    function deposit(uint256 pid, uint256 amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            // Harvest HERMES
            uint256 pending = user.amount.mul(pool.accHermesPerShare).div(ACC_TOKEN_PRECISION).sub(user.rewardDebt);
            HERMES.safeTransfer(msg.sender, pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accHermesPerShare).div(ACC_TOKEN_PRECISION);

        // Interactions
        IRewarder _rewarder = pool.rewarder;
        if (address(_rewarder) != address(0)) {
            _rewarder.onHermesReward(msg.sender, user.amount);
        }
    ...
    /// @notice Harvests HERMES from `MASTER_CHEF_V2` MCJV2 and pool `MASTER_PID` to this MCJV3 contract.
    ...
    function emergencyWithdraw(uint256 pid) external nonReentrant {
        ...
        IRewarder _rewarder = pool.rewarder;
        if (address(_rewarder) != address(0)) {
            _rewarder.onHermesReward(msg.sender, 0);
        }
