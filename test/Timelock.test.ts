...
describe("Timelock", function () {
  before(async function () {
    ...
    this.MasterChefHermesV2 = await ethers.getContractFactory("MasterChefHermesV2")
    this.CustomMasterChefHermesV2Timelock = await ethers.getContractFactory("CustomMasterChefHermesV2Timelock")
  })

  beforeEach(async function () {
    this.hermes = await this.HermesToken.deploy()
    this.timelock = await this.Timelock.deploy(this.bob.address, "259200")
    this.customTimelock = await this.CustomMasterChefHermesV2Timelock.deploy(this.bob.address, "259200", "200", "200", "100", "100")
  })

  it("should not allow non-owner to do operation", async function () {
    await this.hermes.transferOwnership(this.timelock.address)
    // await expectRevert(this.hermes.transferOwnership(carol, { from: alice }), "Ownable: caller is not the owner")

    await expect(this.hermes.transferOwnership(this.carol.address)).to.be.revertedWith("Ownable: caller is not the owner")
    await expect(this.hermes.connect(this.bob).transferOwnership(this.carol.address)).to.be.revertedWith("Ownable: caller is not the owner")
  ...
  it("should do the timelock thing", async function () {
    ...
    await this.timelock
      .connect(this.bob)
      .queueTransaction(this.hermes.address, "0", "transferOwnership(address)", encodeParameters(["address"], [this.carol.address]), eta)
    await increase(duration.days(1))
    await expect(
      this.timelock
        .connect(this.bob)
        .executeTransaction(this.hermes.address, "0", "transferOwnership(address)", encodeParameters(["address"], [this.carol.address]), eta)
    ).to.be.revertedWith("Timelock::executeTransaction: Transaction hasn't surpassed time lock.")
    await increase(duration.days(4))
    await this.timelock
      .connect(this.bob)
      .executeTransaction(this.hermes.address, "0", "transferOwnership(address)", encodeParameters(["address"], [this.carol.address]), eta)
    expect(await this.hermes.owner()).to.equal(this.carol.address)
  })

  it("should also work with MasterChefHermes", async function () {
    this.lp1 = await this.ERC20Mock.deploy("LPToken", "LP", "10000000000")
    this.lp2 = await this.ERC20Mock.deploy("LPToken", "LP", "10000000000")
    this.chef = await this.MasterChef.deploy(this.hermes.address, this.dev.address, this.treasury.address, "100", "0", "200", "200")
  ...
  it("should restrict changes above the limits with MasterChefHermesV2", async function () {
    this.chef = await this.MasterChefHermesV2.deploy(
      this.hermes.address,
      this.dev.address,
    ...
    ).to.be.revertedWith("CustomMasterChefHermesV2Timelock::withinLimits: devPercent must not exceed limit.")
    ...
    ).to.be.revertedWith("CustomMasterChefHermesV2Timelock::withinLimits: treasuryPercent must not exceed limit.")
    await this.customTimelock
      .connect(this.bob)
    ...
    ).to.be.revertedWith("CustomMasterChefHermesV2Timelock::withinLimits: investorPercent must not exceed limit.")
    await this.customTimelock
      .connect(this.bob)
    ...
    ).to.be.revertedWith("CustomMasterChefHermesV2Timelock::withinLimits: hermesPerSec must not exceed limit.")
    await this.customTimelock
      .connect(this.bob)
    ...
    expect(await this.chef.hermesPerSec()).to.equal("99")
  })
