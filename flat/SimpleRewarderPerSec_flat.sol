...
/**
 * This is a sample contract to be used in the MasterChefHermes contract for partners to reward
 * stakers with their native token alongside HERMES.
 *
 * It assumes no minting rights, so requires a set amount of YOUR_TOKEN to be transferred to this contract prior.
 * E.g. say you've allocated 100,000 XYZ to the HERMES-XYZ farm over 30 days. Then you would need to transfer
...
contract SimpleRewarderPerSec is IRewarder, BoringOwnable, ReentrancyGuard {
    ...
    IMasterChefHermes public immutable MCJ;
    ...
    modifier onlyMCJ() {
        require(msg.sender == address(MCJ), "onlyMCJ: only MasterChefHermes can call this function");
        _;
    }
    ...
    constructor(
        ...
        IMasterChefHermes _MCJ,
        bool _isNative
    ) public {
        require(Address.isContract(address(_rewardToken)), "constructor: reward token must be a valid contract");
        require(Address.isContract(address(_lpToken)), "constructor: LP token must be a valid contract");
        require(Address.isContract(address(_MCJ)), "constructor: MasterChefHermes must be a valid contract");
    ...
    /// @notice Function called by MasterChefHermes whenever staker claims HERMES harvest. Allows staker to also receive a 2nd reward token.
    /// @param _user Address of user
    /// @param _lpAmount Number of LP tokens the user has
    function onHermesReward(address _user, uint256 _lpAmount) external override onlyMCJ nonReentrant {
        updatePool();
